...
git apply vm/regression/regression_dune.patch

echo "Patching regression/test*.t files..."
find regression -name "test*.t" -type f -exec perl -pi -e 's/^(\s*)\$\s+\.\.\/src\/Driver\.exe\s+-runtime\s+\.\.\/runtime\s+-I\s+\.\.\/stdlib\/x64\s+-i\s+(test\d+)\.lama\s+<\s+\2\.input\s*$/$1\$ ..\/src\/Driver.exe -runtime ..\/runtime -I ..\/stdlib\/x64 -b $2.lama && ..\/vm\/vm $2.bc < $2.input\n/;' {} \;
